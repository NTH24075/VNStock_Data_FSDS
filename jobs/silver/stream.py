...
def run_silver_stream(
    ...
    checkpoint_dir: str = "data/checkpoints/silver_stream",
    watermark_sec: int = 60,
):
    spark = get_spark("silver_stream", {
    ...
    deduped = (
        parsed.withColumn(
            "_rn",
            F.row_number().over(
                F.window.partitionBy("event_id").orderBy(F.desc("created_ts_parsed"))
            ),
        )
        .filter(F.col("_rn") == 1)
        .drop("_rn")
    ...
    trades = deduped.filter(F.col("event_type") == "trade").select(
        "event_id", "event_ts", "created_ts_parsed", "session_type",
        "ticker", "exchange", "price", "quantity", "side", "trade_id",
        "source_offset", "kafka_ts", "_ingested_at", "_late_flag",
    )
    quotes = deduped.filter(F.col("event_type") == "quote").select(
        "event_id", "event_ts", "created_ts_parsed", "session_type",
        "ticker", "exchange", "bid_price", "bid_qty", "ask_price", "ask_qty",
        "source_offset", "kafka_ts", "_ingested_at", "_late_flag",
    )
    ...
    q1 = (
        trades.writeStream.format("delta").outputMode("append")
        .option("checkpointLocation", os.path.join(checkpoint_dir, "trades"))
        .trigger(processingTime="30 seconds")
        .start(trade_out)
    )
    q2 = (
        quotes.writeStream.format("delta").outputMode("append")
