"""Bronze ingestion — landing Parquet → Bronze Delta tables (PySpark).

Reads generator output from data/landing/, validates against versioned contracts,
writes to Delta Lake with ingest metadata.

Usage:
    python jobs/bronze_ingest.py --data-dir data/landing --bronze-dir data/bronze
"""
...
CONTRACTS_DIR = Path(os.getenv("CONTRACTS_DIR", str(Path(__file__).parent.parent / "contracts")))
