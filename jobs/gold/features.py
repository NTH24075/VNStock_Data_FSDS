...
def build_feat_ticker_daily(spark: SparkSession, gold_dir: str):
    """Compute daily point-in-time features over trading-day row windows."""
    ...
    return_1d = (F.col("close") - F.lag("close").over(window)) / F.lag("close").over(window)

    df = fact.withColumn("return_1d", return_1d)

    df = df.withColumn(
        "f_ticker_return_5d",
        (F.col("close") - F.lag("close", 5).over(window)) / F.lag("close", 5).over(window),
    )

    df = df.withColumn(
        "f_ticker_volatility_20d",
        F.when(
            F.row_number().over(window) >= 20,
    ...
    if os.path.exists(ff_path):
        ...
        if has_ff_cols:
            ff = ff.withColumn(
                "net_foreign", F.col("foreign_buy_value") - F.col("foreign_sell_value")
            )
            df = df.join(
...
def build_feat_ticker_unified(spark: SparkSession, gold_dir: str):
    """Combine daily and intraday feature summaries without future leakage."""
    ...
    if os.path.exists(stream_path):
        ...
        unified = unified.withColumn(
            "f_stream_volume_5m_daily", F.lit(None).cast("long")
